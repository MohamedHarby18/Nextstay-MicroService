...
export default function MyListings() {
  ...
  const listings = (res?.data || []).filter(l => 
  ...
  const { mutate: del } = useMutation({
    ...
    onSuccess: () => { 
      ...
      qc.invalidateQueries(['listings', 'host', userId]) 
    },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
          <div>
            ...
            <p className="page-subtitle">{listings.length} properties</p>
          </div>
          <button onClick={() => navigate('/host/listings/create')} className="btn-primary">
        ...
        <div className="flex items-center gap-3 mb-6 flex-wrap">
          ...
          <div className="flex gap-1 p-1 bg-muted rounded-xl">
            <button onClick={() => setView('grid')} className={`p-2 rounded-lg transition-colors ${view === 'grid' ? 'bg-white shadow-sm' : ''}`}>
              <LayoutGrid size={16} />
            </button>
            <button onClick={() => setView('list')} className={`p-2 rounded-lg transition-colors ${view === 'list' ? 'bg-white shadow-sm' : ''}`}>
              <List size={16} />
            </button>
          </div>
        </div>
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between mt-2">
                    <p className="text-sm font-bold">
                      {formatCurrency(l.pricePerNight)}<span className="font-normal text-text-secondary text-xs"> /night</span>
                    </p>
                  ...
                  <div className="flex gap-2 mt-3">
                    {/* FIXED: View button now navigates to the guest-facing property page */}
                    <button onClick={() => navigate(`/guest/property/${l.id}`)} className="btn-ghost btn-sm flex-1 text-xs">
                      <Eye size={13} /> View
                    </button>
                    {/* FIXED: Edit button now correctly navigates to the host edit route */}
                    <button onClick={() => navigate(`/host/listings/${l.id}/edit`)} className="btn-ghost btn-sm flex-1 text-xs">
                      <Edit2 size={13} /> Edit
                    </button>
                    <button onClick={() => { if (confirm('Delete this listing?')) del(l.id) }} className="btn-ghost btn-sm text-red-500 hover:bg-red-50 text-xs">
                      <Trash2 size={13} />
                    </button>
                  </div>
                </div>
            ...
            {listings.map((l, i) => (
              <div key={l.id} className="card-hover p-4 flex items-center gap-4">
                ...
                <div className="text-right shrink-0">
                  ...
                  <div className="flex items-center gap-1 text-xs font-semibold justify-end mt-1">
                    <Star size={12} className="fill-amber-400 text-amber-400" />
                    {l.averageRating?.toFixed(1) || '—'}
                  </div>
                </div>
                <div className="flex gap-1 shrink-0">
                  {/* FIXED: List view Edit button updated */}
                  <button onClick={() => navigate(`/host/listings/${l.id}/edit`)} className="p-2 hover:bg-muted rounded-xl transition-colors">
                    <Edit2 size={15} />
                  </button>
                  <button onClick={() => { if (confirm('Delete?')) del(l.id) }} className="p-2 hover:bg-red-50 text-red-500 rounded-xl transition-colors">
                    <Trash2 size={15} />
                  </button>
                </div>
              </div>
